...
def hostname_lookup(hostname):
    ...
    if wf.cached_data('hostIdList', max_age=configs.get('update_interval')) is None:
        main.update_caches()

    local_cache = wf.cached_data('hostIdList', max_age=configs.get('update_interval'))
    while local_cache is None:
        time.sleep(1)
